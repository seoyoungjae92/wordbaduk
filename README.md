...
게임물 등급분류를 받으려면 자체등급분류사업자(구글·애플·원스토어)를 거쳐야 하고,
그러려면 안드로이드 패키지가 필요하다. 웹 게임이라 그대로는 못 올린다.

`app/`은 PWA 조건을 갖춰 뒀다 — `manifest.webmanifest`, `sw.js`, 아이콘(192/512/maskable).
게임이 원래 오프라인으로 돌기 때문에 서비스 워커는 캐시 우선 한 겹이면 충분하다.
...
테스터 **12명이 14일 이상** 비공개 테스트에 참여해야 프로덕션 배포를 신청할 수 있다.
계정당 한 번이라 두 번째 앱부터는 해당 없다. 조직 계정은 이 요건이 없지만
D-U-N-S(최대 30일)·조직 이메일·웹사이트가 필요해서 지금은 개인이 빠르다.

원스토어도 자체등급분류사업자라 더 빠를 수 있다. 같은 AAB로 양쪽에 올린다.
